...
class RecommendationEnhancements {
  ...
  static Future<List<MusicRecommendation>> getCollaborativeRecommendations(
    ...
    try {
      final recommendations = <MusicRecommendation, double>{};
      ...
      // Weight by similarity rank so less-similar users do not dominate via popularity.
      final rankedSimilarUsers = similarUserIds.take(5).toList();
      for (int rank = 0; rank < rankedSimilarUsers.length; rank++) {
        final userId = rankedSimilarUsers[rank];
        final similarityWeight = (1.0 - (rank * 0.18)).clamp(0.35, 1.0);
        ...
        for (final doc in userReviews.docs) {
          ...
          if (!reviewedTracks.contains(trackKey)) {
            final rec = MusicRecommendation(
              ...
              genres: review.genres ?? const <String>[],
            );

            final ratingWeight = (review.score / 5.0).clamp(0.0, 1.0);
            final contribution =
                similarityWeight * (0.55 + (ratingWeight * 0.45));
            recommendations[rec] = (recommendations[rec] ?? 0.0) + contribution;
          }
        }
      }

      // Sort by weighted relevance among similar users
    ...
    try {
      ...
      for (final artistName in seedArtists.take(3)) {
        try {
          ...
          final searchQuery =
              'artist:"$artistName" year:$currentYear-${currentYear - 2}';

          final searchResults = await spotify.search.get(searchQuery,
              types: [SearchType.track]).first(2); // Get 2 tracks per artist
                ...
                if (item is Track &&
                  ...
                  try {
                    ...
                    if (genres.isNotEmpty) {
                      debugPrint(
                          'Got ${genres.length} genres (from cache/API) for ${item.name}');
                    }
                  } catch (e) {
                  ...
                  if (genres.isEmpty) {
                    try {
                      if (item.artists!.isNotEmpty &&
                          item.artists!.first.id != null) {
                        final artist =
                            await spotify.artists.get(item.artists!.first.id!);
                        genres = artist.genres ?? [];
                        debugPrint(
                            'Got ${genres.length} artist genres from Spotify for ${item.name}');
                      }
                    } catch (e) {
  ...
  static List<MusicRecommendation> ensureDiversity(
    ...
    for (final rec in recommendations) {
      ...
      final hasNewGenre =
          rec.genres.any((g) => !usedGenres.contains(g.toLowerCase()));
  ...
  static double calculateNoveltyScore(
    ...
    if (preferences.favoriteArtists
        .any((a) => a.toLowerCase().trim() == artist)) {
      score *= 0.5;
    }

    // Reduce novelty if track is similar to saved tracks
    for (final saved in preferences.savedTracks) {
      if (saved.toLowerCase().contains(artist) ||
          saved.toLowerCase().contains(song)) {
        score *= 0.3;
        break;
      }
    }
    ...
    final genreNovelty = recommendation.genres.where((g) {
        ...
        (recommendation.genres.length + 1);
  ...
  static List<MusicRecommendation> balanceRecommendations(
    ...
    scored.sort((a, b) => (b['discoveryScore'] as double)
        .compareTo(a['discoveryScore'] as double));
    final discoveryRecs = scored
        .take((allRecommendations.length * discoveryRatio).round())
        .map((e) => e['rec'] as MusicRecommendation)
        .toList();

    scored.sort((a, b) =>
        (b['safeScore'] as double).compareTo(a['safeScore'] as double));
    final safeRecs = scored
        .take((allRecommendations.length * (1 - discoveryRatio)).round())
        .map((e) => e['rec'] as MusicRecommendation)
        .toList();
